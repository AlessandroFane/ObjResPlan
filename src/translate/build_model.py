...
def compute_model(prog):
    ...
    # Salvataggio degli atomi nella coda con i loro argomenti in un file
    with open("atom-oggetti.txt", 'w') as f:
        for atom in queue.queue:
            arg_list = ', '.join(str(arg) for arg in atom.args)
            f.write(f"{atom.predicate}({arg_list})\n")
