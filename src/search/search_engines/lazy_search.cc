...
void LazySearch::reset()
{
    ...
    for (int i = 0; i < g_operators.size(); i++){
        ...
        if (combined_forbidden_ops.find(g_operators[i]) != combined_forbidden_ops.end())
...
void LazySearch::get_successor_operators(vector<const Operator *> &ops)
{
    ...
    for (int i = 0; i < preferred_operators.size(); i++)
    {
        ...
        if (combined_forbidden_ops.find(*preferred_operators[i]) != combined_forbidden_ops.end())
        {
