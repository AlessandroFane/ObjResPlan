...
void print_branches()
{
    ...
    for (std::map<k_v_pair, Policy *>::iterator it = g_resilient_policies.begin(); it != g_resilient_policies.end(); ++it)
    {
        ...
        for (int i = 0; i < g_variable_domain.size(); i++)
        {
            if (-1 != vars[i])
            {
                const string &fact_name = g_fact_names[i][vars[i]].stringa;
...
void print_resilient_policy_json(map<ResilientNode, Operator> policy)
{
    ...
    for (std::map<ResilientNode, Operator>::iterator it = policy.begin(); it != policy.end(); ++it)
    {
        ...
        {
            const string &fact_name = g_fact_names[j][(it->first.get_state())[j]].stringa;
            if (fact_name != "<none of those>")
                state.push_back(fact_name);
        }
        item["state"] = state;
        ...
        std::set<std::set<Operator> > deactivated_op_ = it->first.get_deactivated_op();

        // Set per raccogliere tutti gli Operator
        std::set<Operator> combined_set;

        // Iterazione attraverso deactivated_op_
        for (std::set<std::set<Operator> >::const_iterator outer_it = deactivated_op_.begin(); outer_it != deactivated_op_.end(); ++outer_it) {
        ...
        for (std::set<Operator>::iterator it_o = combined_set.begin(); it_o != combined_set.end(); ++it_o)
