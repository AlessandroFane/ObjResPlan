...
// Definizione della struttura per la terna
// struct FactInfo {
//     std::string stringa;
//     std::vector<std::string> vettore_stringhe;
//     bool booleano;

//     // Costruttore per semplificare la creazione di una FactInfo
//     FactInfo(const std::string& str, const std::vector<std::string>& vec, bool b) 
//         : stringa(str), vettore_stringhe(vec), booleano(b) {}
// };

// Dichiarazione del vettore g_fact_names
std::vector<std::vector<FactInfo> > g_fact_names;
std::vector<std::string> oggetti_fragili;
std::vector<std::string> oggetti_fragili_input;
...
void check_magic(istream &in, string magic)
{
    ...
    if (in) {
        in >> word;
        if (in.fail()) {
            std::cerr << "Failed to read from input" << std::endl;
        } else {
...
void read_variables(istream &in)
{
    ...
    for (int i = 0; i < count; i++)
    {
        ...
        // Creazione di un vettore di FactInfo per la variabile corrente
        std::vector<FactInfo> variable_facts;
        for (int j = 0; j < fact_names.size(); j++) {
            // Estrai il nome dell'atomo (la sottostringa tra parentesi)
            size_t pos_open = fact_names[j].find('(');
            size_t pos_close = fact_names[j].find(')');
            if (pos_open != string::npos && pos_close != string::npos) {
                string atom_name = fact_names[j].substr(pos_open + 1, pos_close - pos_open - 1);
                // Rimuovi gli spazi dall'atomo
                atom_name.erase(remove_if(atom_name.begin(), atom_name.end(), ::isspace), atom_name.end());
                // Dividi il nome dell'atomo in base al separatore ','
                stringstream ss(atom_name);
                string token;
                vector<string> atom_parts;
                while (getline(ss, token, ',')) {
                    atom_parts.push_back(token);
                }

                // Calcola l'intersezione tra atom_parts e oggetti_fragili
                bool fragile_found = false;
                for (size_t k = 0; k < oggetti_fragili.size(); ++k) {
                    if (std::find(atom_parts.begin(), atom_parts.end(), oggetti_fragili[k]) != atom_parts.end()) {
                        fragile_found = true;
                        break;
                    }
                }

                // Aggiungi le parti dell'atomo al secondo elemento di FactInfo
                FactInfo fact_info(fact_names[j], atom_parts, fragile_found);
                variable_facts.push_back(fact_info);
            } else {
                // Se non ci sono parentesi, crea un FactInfo con un insieme vuoto come secondo elemento
                FactInfo fact_info(fact_names[j], vector<string>(), false);
                variable_facts.push_back(fact_info);
            }
        }


        // Aggiunta del vettore di FactInfo a g_fact_names
        g_fact_names.push_back(variable_facts);
...
{
    //     // Your existing read_everything code with debug prints...

    // std::string magic;
    // while (in >> magic) {
    //     std::cout << "Read magic word: " << magic << std::endl;
    //     // process the magic word...
    // }
    ...
    // }
    // if (in.bad()) {
    //     std::cerr << "Stream is in bad state." << std::endl;
    // }




    //Elimina la parte prima
...
// vector<vector<FactInfo> > g_fact_names;
...
std::set<set<Operator> > g_current_forbidden_ops;
