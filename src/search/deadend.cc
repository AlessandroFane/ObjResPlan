...
void DeadendAwareSuccessorGenerator::generate_applicable_ops(const StateInterface &_curr, vector<const Operator *> &ops)
{
    if (g_detect_deadends && g_deadend_policy)
    {
        ...
        if (g_non_resilient_deadends.find(std::make_pair(g_current_faults, combined_forbidden_ops)) != g_non_resilient_deadends.end())
        {
            Policy *current_forbidden = g_non_resilient_deadends[std::make_pair(g_current_faults, combined_forbidden_ops)];
        ...
        for (int i = 0; i < orig_ops.size(); i++)
        {
            ...
            if (0 == forbidden.count(orig_ops[i]->nondet_index) && combined_forbidden_ops.find(*orig_ops[i]) == combined_forbidden_ops.end())
            {
