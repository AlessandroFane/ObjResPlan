...
struct FactInfo {
    ...
    FactInfo(const std::string& str, const std::vector<std::string>& vec, bool b) 
        : stringa(str), oggetti(vec), fragile(b) {}
};
...
extern std::vector<std::vector<FactInfo> > g_fact_names;
extern std::vector<int> g_axiom_layers;
extern std::vector<int> g_default_axiom_values;
extern std::vector<std::string> oggetti_fragili;
extern std::vector<std::string> oggetti_fragili_input;
...
extern std::set<std::set<Operator> > g_current_forbidden_ops; // set of operators that are forbidden at the current node
