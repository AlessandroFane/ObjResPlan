...
public:
    ...
    std::vector<std::string> obj() const;
    ...
    bool hasIntersection(const std::vector<std::string>& vec1, const std::vector<std::string>& vec2) {
        for (size_t i = 0; i < vec1.size(); ++i) {
            for (size_t j = 0; j < vec2.size(); ++j) {
                if (vec1[i] == vec2[j]) {
                    return true; // Trovata un'intersezione
                }
            }
        }
